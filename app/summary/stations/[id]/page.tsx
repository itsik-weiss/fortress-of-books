...
import {
  ...
  summaryLevelLabels,
  summaryStations,
  type SummaryStationId,
} from "@/lib/summary-cards";
import { cn } from "@/lib/utils";

export function generateStaticParams() {
  return summaryStations.map((station) => ({ id: station.id }));
}

export default async function SummaryStationPage({
  params,
  searchParams,
}: {
  ...
  const pack = summaryCardsForStation(station.id as SummaryStationId);
  ...
  return (
    <div className={cn("min-h-full bg-gradient-to-b", station.soft)}>
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mt-8">
          <CardExplorer
            cards={asFlipCards(pack, summaryStations, summaryLevelLabels)}
            index={query.index}
            showAnswer={query.showAnswer}
            basePath={`/summary/stations/${station.id}`}
          />
        </div>
